...
async function buildSystemPrompt(tripId: string, focusedDayId?: string): Promise<string> {
  ...
  return [
    ...
    `  2. Call get_transit_directions to fetch route possibilities, and ALWAYS present multiple distinct options (e.g. Option 1: High-Speed Train / Frecciarossa, Option 2: Regional Train / Trenitalia Regionale, Option 3: Driving / Car Rental, Option 4: Regional Bus / Flixbus) with duration, frequency, estimated cost/schedule, and key highlights.`,
    ...
    `  5. Give the user the choice between the options, and offer to add their chosen option to the itinerary on the appropriate day via add_place (e.g. category="Transport", name="Train: Brindisi → Lecce", address="Brindisi Station to Lecce Station", notes="35 mins · Trenitalia Regionale", description="📍 Step-by-Step Directions:\\n1. Board Trenitalia Regionale at Brindisi Station towards Lecce (28 mins)\\n2. Arrive at Lecce Station").`,
    ...
    `  1. Call add_booking with confirmation code (reference), provider/host, startAt, endAt, price, currency, address, notes, and for flights the full structured legs array (with carrier, flightNumber, fromCity, fromCode, toCity, toCode, local departTime, local arriveTime, seat, and flightClass).`,
    `  2. For hotel / accommodation bookings spanning multiple days, add_booking automatically creates itinerary entries for EVERY day of the reservation (Check-in on start date, Stay on all intermediate dates, and Check-out on end date), preserves address & check-in/out times, and logs accommodation expense in the budget.`,
    `  3. Report a clear summary of all dates and details added to the itinerary.`,
